...
Requested Forge visualizations preserve source meaning, conditions and approval states while using the format that fits the reader: in-conversation tables or diagrams, available visualization tools, or directly authored HTML. One-time explanations need no Brief file, Project Map, composition JSON or shared renderer. General visualization requests use current app capabilities without a Forge-specific workflow. Freely authored artifacts can be edited directly; default local HTML lives under `.forge/visual-docs/` and stays untracked.

The optional managed renderer serves reproducible tracked Project Handbooks, explicit freshness or deterministic rebuild requests, and updates to existing managed documents. Only this path uses `--prepare`, saved `--composition`, source manifests and generated HTML that must be rebuilt rather than edited. Read `plugins/forge/skills/visual-docs/references/managed-renderer.md` when using it. Existing CLI and source formats remain compatible. Both paths verify meaning and the actual reading surface; completed documents do not refresh merely because their sources changed.
