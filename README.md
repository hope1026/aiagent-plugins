...
| Plugin | Purpose |
|---|---|
| `forge` | Canonical Spec and complexity-aware execution process: durable contract changes use approved Spec Deltas, bounded work can run directly with fresh evidence, and complex work uses optional Execution Plans. Includes TDD, debugging, cross-agent authoring, UI design, tone overlays, and request-only Visual Docs. |
...
| `visual-docs` | Builds request-only Brief, Plan, and Spec views plus a tracked source-backed Project Handbook through one Semantic IR and component system |
