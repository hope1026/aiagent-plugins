...
| Structured file plan — when review, handoff, or recovery needs it, never project SOT | Project-chosen path under `docs/plans/` | yes while retained |
| Optional plan progress and Task detail | Plan-local files when useful | yes while the plan is retained |
| Brief, Plan, or Spec Visual Doc, when explicitly requested | `.forge/visual-docs/<view-id>/view.html` | no |
| Shared handbook, when explicitly requested | User-specified path or project documentation convention | according to project convention; remains derived |
