...
grep -q 'Partial implementation, change, or restoration' "$WRITING_PLANS" || fail "plan scope cannot follow partial work"
grep -q 'Do not write the implementation twice' "$WRITING_PLANS" || fail "plan still duplicates implementation"
grep -q 'same unchanged relevant source, implementation, tests, inputs, settings, and environment' "$VERIFYING_WORK" || fail "verification evidence cannot be reused safely"
grep -q 'Partial implementation of an approved contract' "$VERIFYING_WORK" || fail "partial implementation still requires whole-bundle completion"
grep -q 'Stop after the exact claim, affected contracts, and required project gates are proven' "$VERIFYING_WORK" || fail "verification has no sufficient-evidence stop rule"
grep -q 'Only a claim that a new or never-implemented Canonical Spec is fully implemented' "$USING_FORGE" || fail "router does not separate work completion from bundle completion"
grep -q 'one independently reviewable durable condition' "$WRITING_SPECS" || fail "spec authoring still permits unreadable compound headings"
grep -q '^Governing statements:$' "$WRITING_PLANS" || fail "plan template does not match the parser contract"
