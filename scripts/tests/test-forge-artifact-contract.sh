...
grep -q 'Run one build command' "$REVIEW_VIEWER" || fail "visual-docs build-once contract missing"
...
grep -q 'Visual Docs tooling' "$ROOT/plugins/forge/skills/verifying-work/SKILL.md" || fail "tooling verification boundary missing"
