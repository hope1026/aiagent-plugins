"""Deterministic adaptive renderer for requested Forge Visual Docs."""
...
from review_sources import BriefDocument, PlanDocument, ReviewBundle, ReviewSource
...
def _document_source_rows(bundle: ReviewBundle) -> list[dict[str, object]]:
    return [
        {
            "key": _internal_source_key(source),
            "role": source.role,
...
def manifest_source_records(bundle: ReviewBundle) -> dict[str, list[dict[str, object]]]:
    ...
    return {
        ...
        "document_sources": _document_source_rows(bundle),
    }
...
def _primary_title(bundle: ReviewBundle) -> str:
    ...
    if isinstance(source.document, (BriefDocument, ProjectMap)):
        return source.document.title
    return source.title
...
def _view_context(bundle: ReviewBundle, locale: str) -> ViewContext:
    source = bundle.primary[0]
    if source.spec_bundle is not None:
        spec_kind = source.spec_bundle.metadata.kind
        subtype = source.spec_bundle.metadata.subtype
    else:
        spec_kind = bundle.kind
        subtype = None
    return ViewContext(
        bundle.kind,
        spec_kind,
        subtype,
        "execution" if bundle.kind == "plan" else "review",
        "mixed",
        locale,
        "standalone",
    )
...
def render_review(
    ...
    if bundle.kind not in {"brief", "plan", "spec", "project"}:
        raise ValueError("Visual Docs kind must be brief, plan, spec, or project")
    if locale not in LABELS:
        raise ValueError("Visual Docs locale must be en or ko")
    expected_source_base = "../../" if bundle.kind == "project" else "../../../"
    if source_base != expected_source_base:
        raise ValueError(
            f"Visual Docs source_base must be {expected_source_base} for {bundle.kind}"
        )
    if not _source_sequence(bundle):
        raise ValueError("Visual Docs requires at least one source")
