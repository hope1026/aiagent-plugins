...
5. Self-review language, EARS discipline, one-contract-per-heading readability, member boundaries, `Documents` completeness, full Requirement wording, the decision to use or omit Acceptance Criteria, ambiguity, and source-owned Mermaid. When changing a public interface, authorization, persisted format, or cross-component responsibility, inspect the owning bundle's declared relations and known consumers that repeat the contract; align them inside the approved scope. When Acceptance Criteria are present, also review full Acceptance wording, exact Requirement links, and complete Requirement coverage. Reject Requirement and Acceptance placeholders that only defer to another source.
...
2. Leave the existing bundle bytes and authority unchanged. Draft a Spec Delta that names the baseline bundle SHA-256 and links every affected Requirement and any affected Acceptance statement by member path and exact heading. State exact additions, modifications, removals, member moves, and the proposed current decision summary. For public interfaces, authorization, persisted formats, and cross-component responsibilities, include any owning or consuming bundle whose repeated contract meaning must change; do not turn this into an unrelated repository-wide audit.
...
| "The validator passed, so the prose is good." | Self-review owns direct Requirement meaning, scope, and any observable Acceptance statements. |
| "A source-matches Acceptance proves traceability." | It adds no observable contract. Omit Acceptance Criteria or write a real scenario that covers the Requirement. |
